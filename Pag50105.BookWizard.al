    ...
    layout
    {
        area(content)
        {
            ...
            group(Step1)
            {
                ...
                group("Welcome to Book")
                {
                    Caption='Welcome to Book Setup';
                    Visible=Step1Visible;
                    group(Group18)
                    {
                        Caption='';
...
local procedure StoreBook();
var
    Book : Record Book;
begin
    IF NOT Book.GET THEN BEGIN
        Book.INIT;
        Book.INSERT;
    END;

    Book.TRANSFERFIELDS(Rec,FALSE);
    Book.MODIFY(TRUE);
end;
